...
setMethod(f="plotCompareScores", 
          ...
          definition=function(object, x_name, y_name, ...){
            ...
            {stop(paste("Provided y_name was not unique"))}
              ...
              x<-(-log(abs(as.numeric(slot(slot(object, "score_data"), 
             "pval_data")[[grep(x_name, colnames(slot(slot(object, "score_data")
                        , "pval_data")))]])))*sign(slot(slot(object, 
                           "score_data"), "effect_data")[[grep(x_name, 
                          colnames(slot(slot(object, "score_data"), 
                                        "effect_data")))]]
                          ))
...
setMethod(
                        ...
                        comparePlot=FALSE,pdfOut=NULL)
    {
        ...
        if(!is.null(pdfOut)){
            ...
            else {pdf(pdfOut, width=16, height=8.5)}
        }
        ...
        vect2color = function(v, palettev, breaks) {
            w = v; 
            for (i in 1:length(palettev)) { w[which( v >= breaks[i] & v < 
                                            breaks[i+1] )] = palettev[i]; }
            return(w);
        }
    ...
    if(any(suppressDetails == FALSE,counter==1)){
        ...
        text(-1.25+1.2*cos(seq(start, 
            start+delta*2, len=30)[15]), 1.25+.4*.75*sin(seq(start, 
            start+delta*2, len=30)[15]), "expression", cex=.6)
        ...
        for(j in slot(slot(object, "score_data"), "signsindex")[, 3]){
            ...
            for(g in 1:4){
                ...
                arctext(x=-1.25, y=1.25, r=.35, start=start+delta2*(g-1), 
                                    ...
                                    names(methcol)[g], 
                                    substring(names(methcol)[g], 1, 1)), cex=.5)
            }
    ...
    vertexPalette.v[which(vertexBreaks.v>=min(V(h)$weight[which(V(h)$weight>=
    ...
    if(length(which(E(h)$weight>=
                        qchisq(1-p_thresh,4)))>0){
        edgePalette.v[which(edgeBreaks.v>=min(E(h)$weight[which(E(h)$weight>=
...
addShadowText(layout1_scaled[, 1], layout1_scaled[, 2], vl, font=2, 
    ...
    rescale(stat.v, to=(c(.5, 2)))}else{.5},bg="white",col="black")
...
if(namestyle == "refseq"){
    ...
    text(layout1_scaled[, 1], layout1_scaled[, 2]-.05, sapply(vl, function(k){
...
if(namestyle == "symbol"){
    ...
    "\nChi-square P-value=", round(slot(slot(object, "score_data"), 
    ...
    if(nrow(slot(slot(object, "score_data"), 
        ...
        for(i in 1:nrow(slot(slot(object, "score_data"), 
            ...
            "module_output")$goseqOut[[n_plot]][i, 4], x=1.3, 
                 y=seq(1.4, -1, length.out=
                           nrow(slot(slot(object, "score_data"),
                            "module_output")$goseqOut[[n_plot]]))[i], 
                 adj = c(0, 0))
            text(slot(slot(object, "score_data"), 
                "module_output")$goseqOut[[n_plot]][i, 6], 
                 x=1.5, y=seq(1.4, -1, length.out=
                                nrow(slot(slot(object, "score_data"),
                "module_output")$goseqOut[[n_plot]]))[i], 
                 adj= c(0, 0))
...
setMethod(f="plotDensityPval", 
          ...
          definition=function(x, ref="expression_pvalue", ...){
            ...
            if(!any(grepl(ref,colnames(slot(slot(x, "score_data"), 
              ...
              dens_range<-lapply(slot(slot(x, 
                                          "score_data"), "pval_data"),
                                
                     function(i){dens<-density(as.numeric(i), 
                        na.rm=TRUE)
                        x<-dens$x
                        y<-dens$y
            ...
            if(!names(dev.cur())%in%c("RStudioGD")){par(mar=c(4,4,4,10), xpd=FALSE)
              ...
              sapply(slot(slot(x, "score_data"), "signsindex")[, 3], 
                  ...
                  if(!all(is.na(slot(slot(x, "score_data"), "pval_data")[[
                      ...
                      lines(density(abs(as.numeric(slot(slot(x, "score_data"), 
                        "pval_data")[[grep(i, colnames(slot(slot(x, "score_data"
                         ), "pval_data")))]])), na.rm=TRUE), col= which(slot(
                             slot(x, "score_data"), "signsindex")[, 3] == i))})
              lines(density(abs(
