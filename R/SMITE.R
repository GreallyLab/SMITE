...
setMethod(
                        ...
                        strand_col=NULL, gene_name_col=NULL)
    {
        ...
        if(!is.null(other_data)){if(length(other_tss_distance) != length(other_data))
        {other_tss_distance<-rep(other_tss_distance[1], length(other_data))}
        if(is.null(names(other_tss_distance))){names(other_tss_distance)<-otherdata_names}
        }
        ...
        if(!is.null(other_data)){ suppressWarnings(
...
setMethod(
    ...
    definition=function(annotation, modData, weight_by=NULL, 
        ...
        mylist<-lapply(mylist, function(each_feature){		
            ...
            new_pval <- replace(new_pval, new_pval==0, min(subset(new_pval, new_pval!=0), na.rm=T))
        ...
        colnames(final)<- paste(modType, apply(expand.grid(c("effect", "pvalue")
                                                           , modInclude), 1, function(i){paste(i[2], i[1], 
                                                                                               sep="_")}), sep="_")
                   ...
                   for(x in subset(names_temp_pval_data,
                       ...
                       if(!all(is.na(p_temp))){
                           ...
                           each<-t(sapply(c(seq(.05, .95, .05), rev(1/seq(.05,
                                                                          .95, .05))),
                                          function(i){c(i, wilcox.test(logit_ref, 
...
setMethod(
    ...
    definition=function(object, weights){ 
        ...
        rand_mat<-as.matrix(sapply(1:100, function(j){ sample(scoresout, 
                                                              replace=TRUE)}
        ))
...
setMethod(
    ...
    {        
        ...
        if(any(is.infinite(W_vec))){
            W_vec[which(is.infinite(W_vec))]<-max(subset(W_vec[!
                                                                   is.infinite(W_vec)))
        }
        ...
        spin.glass.out = lapply(sig_genes, function(j){ 
            ...
            genes_in_network[cluster_spinglass(network, weights = 
    ...
    definition=function(object, p_thresh=0.05, coverage, type="reactome")
    {
        ...
        if(any(duplicated(a[, 1]))){
            ...
            a<-lapply(a, function(i){
                if(nrow(i)>1){i<-i[which(i[, 2] == max(i[, 2])), ]};i
            })
            a<-do.call(rbind, a)
        }
    ...
    {options(stringsAsFactors=FALSE)
     ...
     nums<-which(do.call("c", lapply(slot(slot(object, "score_data"),
                                          "module_output")$goseqOut, function(each){
                                              any(grepl(ifelse(wholeword == FALSE, searchstring, paste("\\b", 
     ...
     if(length(nums)>0){ 
         ...
         out<-lapply(nums, function(i){
             ...
             cbind(outterm,as.data.frame(pos), as.data.frame(
                 rep(tot,length(pos))))
         }
         )
...
setMethod(
    ...
    definition=function(annotation,alpha=0.05){
        ...
        rand_mat<-as.matrix(sapply(1:100, function(j){ sample(temp, 
                                                              replace=TRUE)}))
