...
setMethod(
    ...
    definition=function(p, w)
    {
        ...
        p_val <-(1-pnorm(Z))*2
        if(p_val<.0000000000000001){p_val<-.0000000000000001}
        if(p_val>.9999999999999999){p_val<-.9999999999999999}
...
setMethod(
    ...
    definition=function(outdata, column)
    {
        if(any(outdata[, column] == 0)){
            outdata[which(outdata[, column] == 0), column]=
                min(outdata[-which(outdata[, column] == 0), column], na.rm=TRUE)
        }
        return(outdata)
    }
)
...
setMethod(
    ...
    signature="data.frame", 
    definition=function(data, otherdata=NULL, other_d=10000, 
                        ...
                        strand_col=NULL, gene_name_col=NULL)
    {
        ...
        if(any(duplicated(mcols(data_grange)$genenames)))
        {
            ...
            data_grange=
                data_grange[-which(duplicated(mcols(data_grange)$genenames))]
        }
        
        mcols(data_grange)$feature<-"original"
        
        if(!is.null(otherdata)){
        if(is.null(names(otherdata))){
            otherdata_names<-paste("other", 1:length(otherdata), sep="")
            names(otherdata)<-otherdata_names
        }
        else{
            otherdata_names<-names(otherdata)
        }
        
        }
        if(!is.null(otherdata)){if(length(other_d) != length(otherdata))
            {other_d<-rep(other_d[1], length(otherdata))}
        if(is.null(names(other_d))){names(other_d)<-otherdata_names}
        }
        ...
        if(!is.null(otherdata)){
        other<-do.call(c, sapply(1:length(otherdata), function(i){
            tempother<-c(GRanges(seqnames=otherdata[[i]][, 1], 
                                 ranges=IRanges(start=otherdata[[i]][, 2], 
                                                end=otherdata[[i]][, 3])))
            tempother<-unique(tempother)
            
            
            suppressWarnings(
                overlap<- findOverlaps(flank(data_grange, 
                                             other_d[otherdata_names[i]], 
                                             start=TRUE), tempother)
            )
            tempother<-tempother[as.numeric(subjectHits(overlap))]
            mcols(tempother)$genenames=
                mcols(data_grange)$genenames[queryHits(overlap)]
            mcols(tempother)$feature=otherdata_names[i]
            return(tempother)
        })
        )
        }
        promoters_downstream<-flank(data_grange, -promoter_downstream, 
            start=TRUE)
        promoters_upstream<-flank(data_grange, promoter_upstream, start=TRUE)
       ...
       if(!is.null(otherdata)){ suppressWarnings(
            combined_data<-c(data_grange, promoters, body, other, tss))
       }
       else{suppressWarnings(
           combined_data<-c(data_grange, promoters, body, tss))
       }
        combined_data<-split(combined_data, mcols(combined_data)$genenames)
        slot(combined_data, "metadata")$params<-
            c(
                gene_name_col=gene_name_col, 
                promoter_upstream_distance_tss=promoter_upstream, 
                promoter_downstream_distance_tss=promoter_downstream, 
                other_annotation_distance_tss=other_d
            )
        slot(combined_data, "metadata")$sizes_summary<-lapply(
            ...
            return(c(summary(each_length), sds=sd(each_length)))
            })
        return(new("PvalueAnnotation", annotation=combined_data))
    })
...
setMethod(
    ...
    definition=function(a, a_type, b_type, delim=NULL, verbose=FALSE)
    {
        ...
        if(a_type == "refseq"){
            genes_old <- genes_old[ which(genes_old %in% 
        ...
        out<-out[which(!duplicated(out[,1])),]
...
setMethod(
    ...
    if(any(!c(-1,1)%in%unique(sign(expdata[,effect_col])))){message(
        "WARNING: Effects should provide a direction, but these effects
    ...
    stop("Cannot determine p.value column. Please specify with arg:effect_col")}
     ...
     if(any(expdata[,pval_col]<0,expdata[,pval_col]>1)){stop(
        ...
        phenoData(slot(annotation, "expression"))=
            new("AnnotatedDataFrame", 
                data=as.data.frame(exprs(slot(annotation, "expression"))[, 
...
setMethod(
    ...
    definition=function(annotation, modData, weight_by=NULL, 
        ...
        ##no weights provided
        if(missing(weight_by)){weight_by<-rep("pval", length(unique(mcols(
            unlist(
            slot(annotation, "annotation")))$feature)[!unique(mcols(
            unlist(slot(annotation, "annotation")))$feature)%in%c("original", 
                                                                     "tss")]))
        }
        ...
        if(any(!c(-1,1)%in%unique(sign(modData[,4]     )))){message(
            "WARNING: Effects should provide a direction, but these effects
        are all in the same direction.")}
        if(any(modData[,5]<0,modData[,5]>1)){stop(
            "P-values must be between 0 and 1")}
        ...
        tempannotation<-unlist(slot(annotation, "annotation"))
        overlap_sub<-findOverlaps(tempannotation, mod_grange)
        out<-mod_grange[subjectHits(overlap_sub)]
        mcols(out)<-cbind(mcols(tempannotation[as.numeric(
            queryHits(overlap_sub))]), mcols(out))
        out<-split(out, mcols(out)$genenames)
        tempannotation<-split(tempannotation, mcols(tempannotation)$genenames)
        
        if(modCorr == TRUE){
            if(verbose == TRUE){ message("Computing correlation matrices")}
            ...
            mod_grange_corr2<-lapply(mod_grange_corr, function(j)
                ...
                return(
                    ...
                    return(cor(qnorm(1-j$pval[index]), qnorm(1-j$pval2[index])))
                    })))
                    )
                })
        ...
        mylist<-sapply(modInclude, function(i){
            ...
            #if only one pval
            else{out_mean<-each_effect; out_pval<-each_pval}
                
                return(c(out_mean, out_pval, each_length))
            })
            
            return(do.call(rbind, forreturn))
        })
        
        if(verbose == TRUE){ message("Quantile permuting scores")}
        mylist<-lapply(mylist, function(each_feature){		
            ...
            trans_p[which(is.infinite(trans_p[, 1])), 1]<-max(
                trans_p[-which(is.infinite(trans_p[, 1])), 1])}
            num_list<-split(trans_p$trans, trans_p$cats)
            rand_list<-sapply(1:length(num_list), function(i){
                return(as.matrix(sapply(1:500, function(j){ 
                    return(sample(num_list[[as.numeric(i)]], replace=TRUE))
                    })))
            })
            ...
            each_feature<-replaceZeros(each_feature, 2)
            return(each_feature)
        })
        
        if(verbose == TRUE){ message("Scores have been adjusted")}
        ...
        colnames(final)<- paste(modType, apply(expand.grid(c("effect", "pvalue")
                        , modInclude), 1, function(i){return(paste(i[2], i[1], 
                                                           sep="_"))}), sep="_")
...
setMethod(
    ...
    definition=function(annotation, modType="methylation"){
        ...
        slot(temp,"metadata")$elementnames, "_"),
            function(i)return(i[1])))[, 1]%in%modType)]
...
setMethod(
    ...
    definition=function(object, effect_directions=NULL) {
        ...
        if(is.null(effect_directions)){effect_directions<-
         rep("bidirectional", length(slot(slot(object, "modifications"),
        ...
        if(exp_ind == 1){
            data<-data[, c(as.numeric(sapply(names(effect_directions), 
                 function(i){grep(i, colnames(data))})), grep("exp", 
                                                              colnames(data)))]
        }else{data<-data[, as.numeric(sapply(names(effect_directions), 
                                            function(i){grep(i, 
                                                             colnames(data))}))]
        }
...
setMethod(
    ...
    definition=function(object, trans, ref="expression_pvalue", 
                        method="rescale"){
        if(nrow(slot(slot(object,"score_data"),"pval_data"))==0){stop(
            "Run makePvalueObject function first.")}
        
        
        if(!any(grepl(ref,colnames(slot(slot(object, "score_data"), 
            "pval_data"))))){
            stop("paste(Reference is not one of the available:", 
                 colnames(slot(slot(object, "score_data"), "pval_data")))}
        if(length(grep(ref,colnames(slot(slot(object, "score_data"), 
            "pval_data"))))>1){
            stop("Reference was not specific enough.")}
        
        ref_index<-grep(ref,colnames(slot(slot(object, "score_data"),
            "pval_data")))
        ...
        if(method%in%c("Box-Cox", "box-cox", "boxcox", "Boxcox")){
            if(missing(trans)){
                ...
                expos<-c()
                for(x in colnames(slot(slot(object, "score_data"), "pval_data"))
                    [-which(colnames(slot(slot(object, "score_data"), 
                                          "pval_data"))%in%ref)]){
                    
                    p_temp<-slot(slot(object, "score_data"), "pval_data")[[x]]
                    if(!all(is.na(p_temp))){
                        logit_temp<-log(p_temp/(1-p_temp))
                        each<-t(sapply(c(seq(.05, .95, .05), rev(1/seq(.05,
                                                                    .95, .05))),
                        function(i){return(c(i, wilcox.test(logit_ref, 
                    ...
                    slot(slot(object, "score_data"), "pval_data")[[x]]<-p_temp
                ...
                colnames(slot(slot(object, "score_data"), "pval_data"))))){
                    ...
                    expos<-trans 
                    names(expos)<-colnames(slot(slot(object, "score_data"), 
                    "pval_data"))[-which(colnames(slot(slot(object,
                                        "score_data"), "pval_data"))%in%ref)]}
            ...
            if(method%in%c("Rescale", "rescale")){
                ...
                for(i in slot(slot(object,"score_data"),"signsindex")[, 3]){
                    ...
                    colnames(slot(slot(object, "score_data"), "pval_data")))]])
                        ...
                        {
                        p_temp<-slot(slot(object, "score_data"), 
                        "pval_data")[[grep(i, colnames(slot(slot(object, 
                                                "score_data"), "pval_data")))]]
                        logit_temp<-log(p_temp/(1-p_temp))
                        logit_temp<-rescale(logit_temp, to=range(logit_ref, 
                                                                 na.rm=TRUE))
                        slot(slot(object, "score_data"), "pval_data")[[grep(i, 
                        colnames(slot(slot(object, "score_data"), "pval_data"))
...
setMethod(
    ...
    definition=function(object, weights){ 
        ...
        if(missing(weights)){
            ...
            {
stop("ERROR: Number of factors(with expression data) and weights must equal!")}
            else { 
        ...
        if(any(grepl("exp", names(weights)))){weight_names_temp<-
                                weight_names_temp[-grep("exp", names(weights))]}
        ...
        if(any(scoresout == 0, na.rm=TRUE)){
            scoresout[which(scoresout == 0)]<-min(scoresout[-which(
                scoresout == 0)], na.rm=TRUE)}
        scoresout<-(-2*log(scoresout))
        
        
        
        rand_mat<-as.matrix(sapply(1:100, function(j){ return(sample(scoresout, 
                                                                replace=TRUE))}
                                   ))
        new_pval<-sapply(scoresout , function(i){ 
            return(length(which(rand_mat>as.numeric(i)))/(100*length(scoresout)
                                                          ))
            })
        new_pval<-replaceZeros(data.frame(new_pval), 1)
        new_pval<-(-2)*log(new_pval[, 1])
        
        
        
        slot(slot(object, "score_data"), "scores")<-data.frame(scores=
        as.numeric(new_pval), row.names=as.character(slot(slot(object,
                                                    "score_data"), "genes")))
        return(object)
    }
) 
...
setMethod(
    ...
    {        
        ...
        genes_in_network=subset(slot(slot(object, "score_data"), "genes"), 
            slot(slot(object, "score_data"), "genes")%in%V(network)$name)
        ...
        if(any(is.na(scores_in_network))){
            ...
            scores_in_network<-scores_in_network[-which(is.na(
                scores_in_network))]
        ...
        temp1 = apply(network.adj, 1, function(v) return(v*stat_scores))
        W =(temp1 + t(temp1))
        rm(temp1)
        gc()
        W_vec<-(-2*log(1-pchisq(as.vector(W),4)))
        if(any(is.infinite(W_vec))){
            W_vec[which(is.infinite(W_vec))]<-max(W_vec[-which(
                is.infinite(W_vec))])
        }
        ...
        gc()
        final_network<-graph_from_adjacency_matrix(W, mode = "undirected", weighted=TRUE)
        ...
        sig_genes<-genes_in_network[which(pval_scores<node_alpha)]
        ...
        spin.glass.out = lapply(sig_genes, function(j){ 
            ...
            return(genes_in_network[cluster_spinglass(network, weights = 
                            E(network)$weight, vertex=j, gamma=gam)$community])
        })
        names(spin.glass.out) = sig_genes 
        
        ## Select modules with size requirements ##
        spin.size = do.call(c, lapply(spin.glass.out, length))
        spin.glass.out = spin.glass.out[which(spin.size >= minsize & spin.size 
            <= maxsize)]
        
        Modularity.edges = function(v, network)
        { 
            h = induced_subgraph(network, v);
            return(c(sum( E(h)$weight )))
        }
        edge_sum = do.call(c,lapply(spin.glass.out, function(j) { return(
        ...
        random_edges= lapply(1:nspin.glass.out, function(i) {
            ...
            v<-sapply(1:niter, function(k){
                ...
                W<-apply(W, 2, function(i){ i[which(i>0)]<-
                    (-2*log(1-pchisq(i[which(i>0)],4))); return(i)})
                return(sum(W)/2)
            })
            return(v)
        })
        ...
        random_p<-lapply(1:nspin.glass.out, function(k){
            return(length(which(random_edges[[k]] >
                                    edge_sum[k]))/niter)})
        ...
        output<-list();
        output[[1]] = spin.glass.out[which(do.call(c,random_p)<random_alpha)];
        ...
        output[[5]] = 
            lapply(output[[1]], function(i){
                stat.mod<-sum(abs(stat_scores[which(genes_in_network%in%i)]))
                pval.mod<- 1-pchisq(stat.mod,2*length(i))
                return(c(statMod=stat.mod, pvalMod=pval.mod, lenMod=length(i)))
                })
...
setMethod(
    ...
    definition=function(object, network, alpha = 0.05)
    {
       ...
       output<-list();
        ...
        names(output) = c("modules",  "p_value", 
                          "statistic","network","moduleStats","moduleType");
           sub_score<-scores[which(names(scores)%in%output[[1]][[1]])]
           stat.mod<-sum(abs(sub_score))
           pval.mod<- 1-pchisq(stat.mod,2*length(output[[1]][[1]]))
           output[[5]]<-
     list(c(statMod=stat.mod, pvalMod=pval.mod, lenMod=length(output[[1]][[1]]))
          )
       
       
     names(output[[1]])[1]<-names(sub_score[which(sub_score==max(sub_score))[1]]
                                  )
     slot(slot(object,"score_data"),"module_output")<-output
    return(object)
    }    
...
setMethod(
    ...
    {
        while(!names(dev.cur())%in%c("pdf","null device")){ dev.off()}
        ...
        eg2reactome<-AnnotationDbi::as.list(reactome.db::reactomeEXTID2PATHID)
        temp<-sapply(names(eg2reactome), function(y){
            return(eg2sym[[as.character(y)]])})
        temp<-lapply(temp, function(y){if(is.null(y)){y<-NA};return(y)})
        ...
        nl[, 1]<-gsub("[0-9]*$", "", nl[, 1])
        nl<-split(nl[, 1], nl[, 2])
        nl<-lapply(nl, function(i){return(unique(i))})
        
        
        if(class(coverage) == "data.frame"){
            a<-coverage
            a<-a[, 4:5]
        }
        ...
        if(any(duplicated(a[, 1]))){
            ...
            a<-lapply(a, function(i){
                if(nrow(i)>1){i<-i[which(i[, 2] == max(i[, 2])), ]};return(i)
            })
            a<-do.call(rbind, a)
        }
        
        
        slot(slot(object, "score_data"), "module_output")$goseqOut<-lapply(
        slot(slot(object, "score_data"), "module_output")$modules, function(i){
            ...
            path<-goseq(pwf, "hg19", "knownGene", gene2cat=sym2network)
            path<-cbind(path,(as.character(sapply(path$category, function(i){
                return(PATHID2NAME[[i]])}))))
            colnames(path)[6]<-"cat_name"
            return(path[which(path$over_represented_pvalue<p_thresh), ])
        })
        return(object)
    }) 
...
setMethod(
    ...
    {options(stringsAsFactors=FALSE)
        ...
        nums<-which(do.call("c", lapply(slot(slot(object, "score_data"),
                                    "module_output")$goseqOut, function(each){
            return(any(grepl(ifelse(wholeword == FALSE, searchstring, paste("\\b", 
                searchstring, "\\b", sep="")), tolower(each[, ncol(each)]))))}))
        )
        ...
        if(length(nums)>0){ 
            ...
            pos<-grep(
                ifelse(wholeword == FALSE, searchstring, paste("\\b", 
                                        searchstring, "\\b", sep="")), 
            ...
            outterm<-as.data.frame(as.character(slot(slot(object, "score_data"),
                        "module_output")$goseqOut[[i]][pos, 6]))
            return(cbind(outterm,as.data.frame(pos), as.data.frame(
                        rep(tot,length(pos)))))
            }
            )
            ...
            out[,2]<-sapply(out[,2], function(i){i<-as.numeric(i); i <-paste(
                                    i, 
                                    "/", 
        ...
        else{ message("Search term not found.")}
...
setMethod(
    ...
    definition=function(annotation,alpha=0.05){
        ...
        if(any(alpha<0, alpha>1)){stop("alpha must be between 0 and 1")}
        ...
        rand_mat<-as.matrix(sapply(1:100, function(j){ return(sample(temp, 
            replace=TRUE))}))
        new_pval<-sapply(temp , function(i){ 
            return(length(which(rand_mat>as.numeric(i)))/(100*length(temp)
            ))
        })
        new_pval<-replaceZeros(data.frame(new_pval), 1)
        
        return(temp[which(new_pval<alpha)])
