...
author: "Andrew Johnston, N. Ari Wijetunga, and John M. Greally"
date: "July 22, 2015"
output: pdf_document
fontsize: 12pt
vignette: >
...
```{r, warning=FALSE, message=FALSE, eval=T, tidy.opts=list(blank=FALSE, width.cutoff=50), echo=TRUE, results='hide'}
...
To curate the DNA methylation dataset, it is necessary to remove uninformative loci (`NAs`) and p-values=0 since logarithms of p-values are employed within the code.  We replace any p-values=0 with the minimum p-value in the dataset using the provided `replaceZeros` function. We also remove CpGs for which the p-value is `NA`.  The curated datasets available through [Github](https://github.com/GreallyLab/SMITE) have already been processed but were not included in the package because of size restrictions.

```{r, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=60), eval=T}
...
```{r, warning=FALSE, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=60), eval=T}
...
After gene id conversion (example shown above), we remove expression data for genes where a gene symbol was not found (`NAs`). We then take the lowest (most significant) pvalue for genes that had more than 1 entry. Finally, we replace p-values of zeros with the lowest p-value in the dataset using our `replaceZeros` command.  
...
```{r, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=50), eval=T}
...
```{r, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=50), eval=F, fig.width= 9}
## See expression data ##
...
```{r,  tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=35), eval=F}
...
```{r,  tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=35), eval=T}
## Plot density of p-values ##
plotDensityPval(test_annotation, ref="expression")
```
```{r,  tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=35), eval=T, fig.width=9, fig.height=5}
...
```{r, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=35), eval=T, fig.width=8}
...
```{r long-compute-time, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=45), eval=F}
...
```{r, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=60), eval=T, fig.width=10, fig.height=9}
...
```
```{r, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=60), eval=T, fig.width=12,fig.height=6}
